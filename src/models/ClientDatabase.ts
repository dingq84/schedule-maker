...
class ClientDatabase {
	...
	constructor(public databaseName: string, public version: number, public tableName: string) {
	...
	static deleteDatabase() {
		return new Promise((resolve, reject) => {
			const req = window.indexedDB.deleteDatabase('taskDatabase')
			req.onsuccess = () => {
				resolve('success')
			}
			req.onerror = () => {
			...
			req.onblocked = () => {
				reject(new Error("Couldn't delete database due to the operation being blocked"))
			}
		})
	}
	...
	getAllIndex(db: IDBDatabase) {
		...
		return new Promise((resolve, reject) => {
			tx.onerror = () => {
				return reject(new Error(`Transaction not opened due to error: ${tx.error}`))
			}
	...
	updateTimestamp(db: IDBDatabase, data: { key: string; time: number }): Promise<string> {
		return new Promise((resolve, reject) => {
			...
			request.onerror = () => {
				return reject(
					new Error(`There has been an error with retrieving your data: ${request.error}`)
				)
			}
		})
	}
	...
	addData<T extends unknown>(db: IDBDatabase, data: T | T[], originalStore?: IDBObjectStore) {
		return new Promise((resolve, reject) => {
			...
			request!.onerror = () => {
				return reject(new Error(`There has been an error with adding your data: ${request!.error}`))
			}
	...
	deleteData(
		...
		return new Promise((resolve, reject) => {
			...
			const getDataPromises = dataList.map((item) => this.getData(db, item, store))
			...
			Promise.all(getDataPromises)
				.then(() => {
					...
					request!.onerror = () => {
						return reject(
							new Error(`There has been an error with adding your data: ${request!.error}`)
						)
					}
	...
	deleteDataByIndex(db: IDBDatabase, key: string, query: IDBKeyRange) {
		return new Promise((resolve, reject) => {
			...
			cursors.onerror = () => {
				return reject(new Error(`There has been an error with remove your data: ${cursors.error}`))
			}
		})
	}

	updateData(db: IDBDatabase, data: any | any[], originalStore?: IDBObjectStore) {
		return new Promise((resolve, reject) => {
			let dataList: any[] = []
			if (Array.isArray(data)) {
				dataList = data
			...
			const getDataPromises = dataList.map((item) => this.getData(db, item.id, store))
			Promise.all(getDataPromises)
				.then(() => {
					let request: null | IDBRequest = null
	...
	getDataByKey<T extends unknown>(db: IDBDatabase, query?: string): Promise<T[]> {
		return new Promise((resolve, reject) => {
			const tx = db.transaction(this.tableName, 'readonly')
	...
	getDataByIndex<T extends unknown>(
		...
		return new Promise((resolve, reject) => {
			...
			try {
				...
				return reject(new Error(`There has been an error with open your index: ${event}`))
			}
		})
	}
